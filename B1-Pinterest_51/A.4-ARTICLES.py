...
# ✅ Better: export OPENAI_API_KEY="sk-..." ثم:
...
# File paths
RECIPE_EXCEL = '../ALL/B1-Pinterest_51-out/images.xlsx'  # Input Excel with recipes
OUTLINE_EXCEL = '../ALL/B1-Pinterest_51-out/outline_parts.xlsx'  # Optional (unused here, kept for compatibility)

# Sheet and column configuration
RECIPE_SHEET_NAME = 'Sheet1'
RECIPE_COLUMN_LABEL = 'Recipe'

# Article configuration
TOTAL_WORDS = 3000
MAX_TOKENS_PER_PART = 3000
...
_LANG_INGREDIENTS = {
    "English": {"label": "Ingredients",
                "aliases": {"ingredients", "ingredient", "what you need", "you will need", "what you'll need",
...
def insert_image_after_section3(text: str, placeholder="{{image_ing_2}}") -> str:
...
def _enforce_image_spacing(text: str, placeholder="{{image_ing_2}}") -> str:
...
def create_outline(recipe: str, card_context: str) -> str:
    prompt = (
        f"Create a DETAILED, LONG, SEO-optimized outline for a {TOTAL_WORDS}-word recipe article "
        f"based ONLY on the provided Recipe text and the Recipe Card.\n"
        f"- Language: {ARTICLE_LANGUAGE}.\n"
        f"- STRICT DATA FIDELITY: Use ONLY information present in the Recipe for ingredients and steps.\n"
        f"- STRICT CARD FIDELITY: For timings/servings/course/cuisine/calories use the Recipe Card EXACTLY. "
        f"If any card field is N/A, treat it as 'Not specified in the recipe'.\n"
        f"- Use H2/H3 only (no H1).\n"
        f"- IMPORTANT: Do NOT prefix ANY headings with numbers, numerals, or roman numerals.\n"
        f"- The outline MUST start EXACTLY with these H2 labels (no numbering):\n"
        f"  ## Introduction\n"
        f"  ## Recipe Overview\n"
        f"  ## {INGREDIENTS_LABEL}\n"
        f"- Then include AT LEAST 10 additional H2 sections; each H2 should have 2–4 H3 subsections.\n"
        f"- END with a neutral Conclusion summarizing taste/texture/timing in 2–4 sentences. NO CTAs.\n\n"
        f"{card_context}\n"
        f"Recipe:\n{recipe}"
    )
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    f"You are a professional food blog writer. "
                    f"Write SEO-optimized, richly structured outlines in {ARTICLE_LANGUAGE}. "
                    "Never include CTAs. Base ingredients/steps strictly on the Recipe. "
                    "For timings/servings/course/cuisine/calories use the Recipe Card exactly; if N/A say not specified. "
...
def generate_article_part1(outline_part: str, recipe_text: str, card_context: str, overview_lines: str) -> str:
    prompt = (
        f"Write PART 1 of a LONG, detailed SEO-optimized recipe article based ONLY on the provided Recipe text and Recipe Card.\n"
        f"- Language: {ARTICLE_LANGUAGE}.\n"
        f"- TARGET LENGTH for Part 1: 900–1000 words (do not be brief).\n"
        f"- STRICT: Use ONLY ingredients and steps present in the Recipe; no new ingredients, brands, or quantities not present.\n"
        f"- STRICT CARD FIDELITY (MUST): For Prep/Cook/Total/Course/Cuisine/Servings/Calories, use the Recipe Card EXACTLY. "
        f"If any card value is N/A, write 'Not specified in the recipe'. DO NOT estimate or invent.\n"
        f"- The **very first line MUST be a single H1 in the exact form: '# {{Title}}'** with nothing before it.\n"
        f"- H1 Title rules (VERY IMPORTANT):\n"
        f"  • Be CLEAR and SPECIFIC about the recipe.\n"
        f"  • Length: 6–12 words AND roughly 45–70 characters.\n"
        f"  • Include the core recipe name as written in the Recipe.\n"
        f"  • Allowed simple adjectives if true: Easy, Simple, Homemade, Quick.\n"
        f"  • NO emojis, NO quotes, NO brackets, NO pipes/double dashes. Avoid dates/years/clickbait.\n"
        f"  • Use natural Title Case or Sentence case.\n"
        f"- Then follow this EXACT order using NON-NUMBERED headings:\n"
        f"  ## Introduction — 2–4 short paragraphs that set context, taste, and when to serve.\n"
        f"  ## Recipe Overview — MUST use EXACTLY the following bullets (copy them as-is, same order, same labels):\n"
        f"{overview_lines}"
        f"  IMPORTANT: Do NOT add or change any values in the bullets above.\n"
        f"  ## {INGREDIENTS_LABEL}\n"
        f"     - Bullet list of ALL ingredients extracted from the Recipe ONLY.\n"
        f"  ## Instructions — numbered, step-by-step, derived strictly from the Recipe.\n"
        f"- After these, cover the remaining H2/H3 from the outline part with rich paragraphs.\n"
        f"- Do NOT write a conclusion in Part 1. NO calls to action.\n\n"
        f"{card_context}\n"
        f"Recipe:\n{recipe_text}\n\nOutline Part 1:\n{outline_part}"
    )
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                ...
                "content": (
                    f"You are a professional food blog writer. "
                    f"Write long, SEO-friendly recipe articles in {ARTICLE_LANGUAGE}. "
                    "No CTAs. Derive ingredients/steps ONLY from the Recipe. "
                    "Recipe Overview bullets MUST be copied exactly as provided. "
                    "For Prep/Cook/Total/Course/Cuisine/Servings/Calories, you MUST use the Recipe Card exactly; if N/A say not specified. "
                    "NEVER prefix any H2/H3 with numbers or roman numerals."
                )
            },
            {"role": "user", "content": prompt}
        ],
...
def generate_article_part2(outline_part: str, recipe_text: str, card_context: str) -> str:
    prompt = (
        f"Write PART 2 (continuation) of the same LONG article. Do NOT repeat the title.\n"
        f"- Language: {ARTICLE_LANGUAGE}.\n"
        f"- TARGET LENGTH for Part 2: 900–1000 words (do not be brief).\n"
        f"- Use only H2/H3; continue from the outline. Avoid duplicating content from Part 1.\n"
        f"- STRICT: Do NOT invent new ingredients or unprovided timings.\n"
        f"- STRICT CARD FIDELITY: When mentioning timings/servings/course/cuisine/calories, use the Recipe Card values exactly. "
        f"If card has N/A, write 'Not specified in the recipe'.\n"
        f"- Conclusion: neutral 2–4 sentences. NO CTAs.\n"
        f"- IMPORTANT: Do NOT prefix ANY headings with numbers.\n\n"
        f"{card_context}\n"
        f"Recipe:\n{recipe_text}\n\nOutline Part 2:\n{outline_part}"
    )
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    f"Continue a long SEO-optimized recipe article in {ARTICLE_LANGUAGE} using H2/H3 only. "
                    "No CTAs. Do not invent new ingredients or timings. "
                    "For Prep/Cook/Total/Course/Cuisine/Servings/Calories use the Recipe Card exactly; if N/A say not specified. "
                    "NEVER prefix any H2/H3 with numbers or roman numerals."
                )
            },
            {"role": "user", "content": prompt}
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        part1 = insert_image_after_section3(part1, placeholder="{{image_ing_2}}")
        ...
        final_article = _enforce_image_spacing(final_article, "{{image_ing_2}}")
        ...
        # Save .txt
        safe_name = "".join(c if c.isalnum() or c in " _-" else "_" for c in (recipe_hint or "")).replace(" ", "_")[
                    :100]
