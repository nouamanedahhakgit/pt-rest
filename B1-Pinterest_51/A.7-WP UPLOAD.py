...
WP_URL = os.getenv("WP_URL", "https://yumquik.com")
WP_USER = os.getenv("WP_USER", "admin")
WP_APP_PASSWORD = os.getenv("WP_APP_PASSWORD", "VIgJ C2HN AxoW bD9f F6iY Gpw5")
auth = HTTPBasicAuth(WP_USER, WP_APP_PASSWORD)

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

EXCEL_PATH = '../ALL/B1-Pinterest_51-out/images.xlsx'
...
if 'image_4' not in df.columns:
    df['image_4'] = ''
if 'image_ing_2' not in df.columns:
    df['image_ing_2'] = ''
...
for index, row in df.iterrows():
    try:
        ...
        image_4 = row['image_4']
        image_ing_2 = row.get('image_ing_2', '')
        ...
        media_id, media_url = upload_image_to_wordpress(
            image_3, title, first_paragraph_after_heading, first_paragraph_after_heading, first_paragraph_after_heading
        )

        if pd.notna(image_4) and str(image_4).strip():
            image_4_id, image_4_url = upload_image_to_wordpress(
                image_4, title, first_paragraph_after_heading, first_paragraph_after_heading, first_paragraph_after_heading,
                top_crop_pixels=TOP_CROP_PIXELS
            )
            if image_4_url:
                content = content.replace('{{image_4}}', f'<img src="{image_4_url}" alt="{first_paragraph_after_heading}" />')
        if pd.notna(image_ing_2) and str(image_ing_2).strip():
            image_ing_2_id, image_ing_2_url = upload_image_to_wordpress(
                image_ing_2, title, first_paragraph_after_heading, first_paragraph_after_heading, first_paragraph_after_heading
            )
            if image_ing_2_url:
                content = content.replace('{{image_ing_2}}', f'<img src="{image_ing_2_url}" alt="{first_paragraph_after_heading}" />')
